...
@interface StaffSummaryViewController ()<UITableViewDelegate,UITableViewDataSource,QueryCell2Delegate,QueryCell3Delegate,QueryCellDelegate,StaffSummaryFooterDelegate>{
...
@implementation StaffSummaryViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.backgroundColor = PaleColor;
    self.title = @"服务商员工订单汇总";
...
-(void)loadData{
    ...
    QueryModel *queryModel =   _cellArr[0];
...
-(UITableView *)table{
    if (!_table) {
        ...
        [_table registerClass:[StaffSummaryFooter class] forHeaderFooterViewReuseIdentifier:@"StaffSummaryFooter"];
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (section == _cellArr.count -1) {
        StaffSummaryFooter *footer = [tableView dequeueReusableHeaderFooterViewWithIdentifier:@"StaffSummaryFooter"];
...
#pragma mark StaffSummaryFooterDelegate
...
-(void)queryAction{
    ...
    for (NSInteger index = 0; index < _cellArr.count; index++) {
        ...
        for (NSInteger index2  = 0; index2 <queryModel.headCellArray.count ; index2++) {
            ...
            if ([cellModel.titleKey isEqualToString:@"Time_Start"]) {
                ...
                if (cellModel.cellText.length) {//是空就不拼接
    ...
    StaffSumListViewController *sum =[ StaffSumListViewController new];
