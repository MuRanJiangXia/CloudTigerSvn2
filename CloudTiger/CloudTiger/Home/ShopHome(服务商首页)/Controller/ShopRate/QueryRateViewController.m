...
@interface QueryRateViewController ()<UITableViewDelegate,UITableViewDataSource,QueryCellDelegate,QueryCell2Delegate,QueryCell3Delegate,QuerySumFooterDelegate,QueryNewOrderCellDelegate>{
...
@implementation QueryRateViewController
...
-(void)loadData{
    ...
    if (self.queryOrderState == kOrderShop && [cyanManager.isStaff isEqualToString:@"0"] && [cyanManager.customersType isEqualToString:@"1"]) {//是商户
...
-(void)queryAction{
    ...
    //    [self showSVP];
    QueryRateListViewController *query = [QueryRateListViewController  new];
...
-(void)sumAction{
    ...
    [_table endEditing:YES];
    NSMutableDictionary *parameter = [self getParameter];
    RateSummaryViewController *sum =[ RateSummaryViewController new];
