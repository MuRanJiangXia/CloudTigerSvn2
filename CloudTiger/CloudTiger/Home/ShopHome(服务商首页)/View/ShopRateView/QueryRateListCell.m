...
@end
@implementation QueryRateListCell
...
-(void)setResultModel:(ShopRateModel *)resultModel{
...
-(void)layoutSubviews{
    ...
    NSInteger pay =   [_resultModel.Pay_Type integerValue];
    if (pay == 102) {//微信
    ...
    NSString *money =   [CyTools folatByStr:_resultModel.Total_fee];
    _moneyLab.text = [NSString stringWithFormat:@"¥%@",money];
    
    
    _numberLab.text =[NSString stringWithFormat:@"%@", _resultModel.Out_trade_no];
    
    _timeLab.text = [NSString stringWithFormat:@"%@",_resultModel.Time_Start];
