    ...
    [manager POST:_postUrl parameters:self.paramters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (!model.count) {
            ...
            for (NSDictionary* dic in model) {
                QueryResultModel *queryModel = [[ QueryResultModel alloc]init];
                [queryModel setValuesForKeysWithDictionary:dic];
                queryModel.Cash_fee_type = @"CNY";
                [_queryResultArr addObject:queryModel];
            }
...
//        if (_isMove) {
//         [_table setContentOffset:CGPointMake(0,_table.contentOffset.y + 100) animated:YES];
//            _isMove = NO;
//        }
        NSLog(@"end of table");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    QueryResultCell *cell = [tableView dequeueReusableCellWithIdentifier:@"QueryResultCell" forIndexPath:indexPath];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    NSLog(@"点击了。。");
    QueryOrderViewController *query = [QueryOrderViewController new];
    QueryResultModel *model = _queryResultArr[indexPath.row];
