...
//    //@property(nonatomic,copy)NSString *Status;
    ...
    //序号  订单号 交易类型  交易金额 交易币种  交易时间 登录名 真实姓名
    if (self.queryOrderState == kOrderShop) {
         _titleArr = @[@"序号",@"商户名称",@"登录名",@"真实姓名",@"订单号",@"交易类型",@"交易金额",@"折后金额",@"交易币种",@"交易时间"];
        _contArr =    [self.queryModel allPropertyNames];
    }else{
        _titleArr = @[@"序号",@"商户名称",@"订单号",@"交易类型",@"交易金额",@"折后金额",@"交易币种",@"交易时间"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([_titleArr[indexPath.row] isEqualToString:@"交易金额"] ||[_titleArr[indexPath.row] isEqualToString:@"折后金额"] ) {
