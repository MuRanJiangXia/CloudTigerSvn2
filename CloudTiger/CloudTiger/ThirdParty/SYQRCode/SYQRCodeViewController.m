...
//    BlueColor
//    [UIColor colorWithRed:62.0/255 green:199.0/255 blue:153.0/255 alpha:1.0]
    bgView.backgroundColor =   BlueColor;
    [self.view addSubview:bgView];
    
    UILabel *titleLab = [[UILabel alloc] initWithFrame:CGRectMake((kDeviceWidth - 18*7) / 2.0, 28, 18*7, 20)];
...
// rect : {{37.5, 136}, {300, 300}
//          y ， x ， 高 ，宽
    /**修改了识别域，识别域 是按着比例来的（  y ， x ， 高 ，宽）*/

    CGRect rect = CGRectMake( 136/KDeviceHeight, 37.5/kDeviceWidth, 300/KDeviceHeight, 300/kDeviceWidth);
    [output setRectOfInterest:rect];
//    [output setRectOfInterest:[self getReaderViewBoundsWithSize:CGSizeMake(kReaderViewWidth + 100, kReaderViewHeight +100)]];
...
//                [a addObject:AVMetadataObjectTypeUPCECode];
//            
//        }
...
-(void)setDevice{
    ...
    CGPoint  makePoint = self.view.center;
    //更改这个设置的时候必须先锁定设备，修改完后再解锁，否则崩溃
    [self.qrDevice lockForConfiguration:nil];
    //设置闪光灯为自动
    //    [device setFlashMode:AVCaptureFlashModeOn];
    //设置曝光度
    self.qrDevice.exposureMode = AVCaptureExposureModeAutoExpose;
    [self.qrDevice setFocusPointOfInterest:makePoint];
    //设置焦距
   self.qrDevice.focusMode = AVCaptureFocusModeContinuousAutoFocus;
    
    if ([self.qrDevice isFlashModeSupported:AVCaptureFlashModeOn]) {
         [self.qrDevice setExposurePointOfInterest:makePoint];
        [self.qrDevice setFlashMode:AVCaptureFlashModeOn];
    }
...
- (CGRect)getReaderViewBoundsWithSize:(CGSize)asize
{
    ...
    /*
     CGRectMake(CGRectGetMaxX(leftView.frame) - 1,kLineMinY,self.view.frame.size.width - 2 * CGRectGetMaxX(leftView.frame) + 2, kReaderViewHeight + 2)
...
//    
//    UIView *view = [[UIView alloc]initWithFrame:[self getReaderViewBoundsWithSize:CGSizeMake(kReaderViewWidth , kReaderViewHeight)]];
