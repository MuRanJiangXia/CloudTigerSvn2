...
@interface SignViewController ()<UITableViewDelegate,UITableViewDataSource,QueryCellDelegate,QueryCell2Delegate,QueryCell3Delegate,QuerySumFooterDelegate,QueryNewOrderCellDelegate>{
...
- (void)viewDidLoad {
    ...
    if ([_cyanManager.isStaff isEqualToString:@"1"]) {
        if ([_cyanManager.customersType isEqualToString:@"1"]) {//商户下员工
            self.queryOrderState = kOrderShopUser;

            
        }else{//服务商下员工
            self.queryOrderState = kOrderCustomerUser;

            
        }
    }else{
        if ([_cyanManager.customersType isEqualToString:@"1"]) {//商户
            self.queryOrderState = kOrderShop;
            
            
        }else{//服务商
            self.queryOrderState = kOrderCustomer;
...
-(void)queryAction{
    ...
    if ([_cyanManager.customersType isEqualToString:@"0"]&& [_cyanManager.isStaff isEqualToString:@"0"]) {//服务商不判断
        
    }else{
        
        if ([_cyanManager.powers indexOfObject:CyanOrderQuery ] == NSNotFound) {
            
            //没有该权限
            [MessageView showMessage:@"该用户没有订单查询权限"];
            return;
...
-(void)sumAction{
    ...
    sum.hidesBottomBarWhenPushed = YES;
