//
//  QueryRefundNewCell2.m
//  CloudTiger
//
//  Created by cyan on 16/10/17.
//  Copyright © 2016年 cyan. All rights reserved.
//

#import "QueryRefundNewCell2.h"
@interface QueryRefundNewCell2(){
...
@end
@implementation QueryRefundNewCell2
...
-(void)setRefudDesModel:(RefudDesModel *)refudDesModel{
...
-(void)layoutSubviews{
    ...
    NSString *money =   [CyTools folatByStr:_refudDesModel.Cash_fee];
...
//    model.refundStatus = @"不可退款";
    
    
    if ([_refudDesModel.refundStatus isEqualToString:@"退款"]||[_refudDesModel.refundStatus isEqualToString:@"部分退款"]) {
        _stateLab.textColor = UIColorFromRGB(0xff5c11, 1);
    }else if ([_refudDesModel.refundStatus isEqualToString:@"退款完成"]){
        _stateLab.textColor = UIColorFromRGB(0x27a005, 1);
    }
    else{
        _stateLab.textColor = GrayColor;

    }
    _stateLab.text = [NSString stringWithFormat:@"%@",_refudDesModel.refundStatus];
}
-(void)createUI{
    ...
    _numberLab  = [[UILabel alloc]initWithFrame:CGRectMake(_moneyLab.right, _stateLab.bottom +2 , MainScreenWidth -_moneyLab.right  - 15, 35)];
